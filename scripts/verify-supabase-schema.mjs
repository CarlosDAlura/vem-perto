...
const expectedTables = [
  ...
  'coupon_redemptions', 'payments', 'delivery_assignments', 'order_status_history', 'store_favorites',
  'reviews', 'order_messages', 'notifications', 'loyalty_accounts', 'loyalty_transactions', 'audit_logs'
];

const files = (await readdir(migrationsDir)).filter(file => file.endsWith('.sql')).sort();
if (files.join(',') !== '20260718000100_foundation.sql,20260718000200_rls_policies.sql,20260718000300_order_transactions.sql,20260718000400_realtime.sql,20260718000500_stage_02_app_rpcs.sql,20260718000600_fix_approval_status_rpcs.sql') {
...
for (const table of expectedTables) {
  const tableSource = sql['20260718000100_foundation.sql'] + sql['20260718000500_stage_02_app_rpcs.sql'];
  if (!tableSource.includes(`create table public.${table}`) && !tableSource.includes(`create table if not exists public.${table}`)) {
    throw new Error(`Missing central table: ${table}`);
  }
  const rlsSource = sql['20260718000200_rls_policies.sql'] + sql['20260718000500_stage_02_app_rpcs.sql'];
  if (!rlsSource.includes(`alter table public.${table} enable row level security;`)) {
